 ...
 * 用户产品请求
 */
public class CustomerProductRequest extends BaseJsonRequest<CustomerProduct> {
	public CustomerProductRequest(ICallback<CustomerProduct> callback) {
		super(callback);
	}
	...
	public void onResponse(JSONObject response) throws JSONException {
		if (response != null && response.containsKey("response")) {
			...
			if (jo.getString("rspCode").equalsIgnoreCase("0000")) {
				...
				List<CustomerProduct> customerProducts = JSON.parseArray(jo1.toString(), CustomerProduct.class);
				getCallback().onReceive(customerProducts);
			} else
