 ...
 * 用户模式区域请求
 */
public class CustomerAreaRequest extends BaseJsonRequest<CustomerArea> {
	public CustomerAreaRequest(ICallback<CustomerArea> callback) {
		super(callback);
	}
	...
	public JSONObject createRequestBody() throws JSONException {
		...
		String updateTime = SPM.getStr(account, NetConstant.KEY_CUSTOMER_AREA, NetConstant.DEFAULT_UPDATE_TIME);
	...
	public Header createRequestHeader() {
		return new Header(NetConstant.BS_CUSTOMER_AREA_OPERATION,
	...
	public void onResponse(JSONObject response) throws JSONException {
		if (response != null && response.containsKey("response")) {
			...
			if (jo.getString("rspCode").equalsIgnoreCase("0000")) {
				...
				List<CustomerArea> customerAreas = JSON.parseArray(jo1.toString(), CustomerArea.class);
				getCallback().onReceive(customerAreas);
			} else
			...
			String account = AppUtil.getCurrentAccount();
			SPM.saveStr(account,
					NetConstant.KEY_CUSTOMER_AREA, processTime);
		}
	}
