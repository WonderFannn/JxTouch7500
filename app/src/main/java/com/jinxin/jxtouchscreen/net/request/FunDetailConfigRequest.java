 ...
 * 设备配置请求
 */
public class FunDetailConfigRequest extends BaseJsonRequest<FunDetailConfig> {
	public FunDetailConfigRequest(ICallback<FunDetailConfig> callback) {
		super(callback);
	}
	...
	public JSONObject createRequestBody() throws JSONException {
		...
		String updateTime = SPM.getStr(account, NetConstant.KEY_PRODUCT_FUN_DETAIL_LIST, NetConstant.DEFAULT_UPDATE_TIME);
	...
	public Header createRequestHeader() {
		return new Header(NetConstant.BS_FUN_CONFIG,
	...
	public void onResponse(JSONObject response) throws JSONException {
		if (response != null && response.containsKey("response")) {
			...
			if (jo.getString("rspCode").equalsIgnoreCase("0000")) {
				...
				List<FunDetailConfig> funDetailConfigs = JSON.parseArray(jo1.toString(), FunDetailConfig.class);
				getCallback().onReceive(funDetailConfigs);
			} else
			...
			String account = AppUtil.getCurrentAccount();
			SPM.saveStr(account,
					NetConstant.KEY_PRODUCT_FUN_DETAIL_LIST, processTime);
		}
	}
