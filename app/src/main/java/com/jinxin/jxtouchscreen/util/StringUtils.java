...
//	}  
	...
	public static String rgbToHex(String color){
		...
		String redStr = color.substring(0,3);
		int red = Integer.parseInt(redStr);
		String greenStr = color.substring(3,6);
		int green = Integer.parseInt(greenStr);
		String blueStr = color.substring(6,9);
		...
		if((0<=red&&red<=255)&&(0<=green&&green<=255)&&(0<=blue&&blue<=255)){
			...
			colorBuffer.append(redHex);
			String greenHex = Integer.toHexString(green);
			if (greenHex.length() <2){
				greenHex = "0" + greenHex;
			}
			colorBuffer.append(greenHex);
			String blueHex = Integer.toHexString(blue);
			if (blueHex.length() <2){
				blueHex = "0"+ blueHex;
			}
			colorBuffer.append(blueHex);
		}
